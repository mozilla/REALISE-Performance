...
#' @param args the arguments from the hyperparamter tuning
...
prepare.result <- function(data, args, status, error,
    ...
    # dataset hash
    data.hash <- tools::md5sum(args$input)
...
exit.error.multidim <- function(data, args, params) {
    ...
    out <- prepare.result(data, args, status, error, params, NULL, NA)
...
exit.with.error <- function(data, args, params, error) {
    status = 'FAIL'
    out <- prepare.result(data, args, status, error, params, NULL, NULL)
    dump.output(out, args$output)
    quit(save='no')
}
...
exit.success <- function(data, args, params, locations, runtime) {
    status = 'SUCCESS'
    error = NULL
    out <- prepare.result(data, args, status, error, params, locations,
                          runtime)
    dump.output(out, args$output)
}