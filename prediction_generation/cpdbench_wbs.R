...
main <- function() {
    ...
    if (data$original$n_dim > 1) {
        ...
        exit.error.multidim(data$original, raw_args, params)
    }
    ...
    if (!is.null(result$error)) {
        exit.with.error(data$original, raw_args, params, result$error)
    }

    # convert to 0-based indices.
    locations <- as.list(result$locations)

    exit.success(data$original, raw_args, params, locations, runtime)
}
