...
def merge_cpocations(first_method_cplocations, second_method_cplocations):
...
def merge_files(first_json, second_json, first_method, second_method):
    result_json = dict()
    ...
    result_json["parameters"], result_json["args"] = prefix_param_arg(first_json, second_json, first_method, second_method)
    result_json["result"] = dict()
    result_json["result"]["cplocations"] = merge_cpocations(first_json["result"]["cplocations"], second_json["result"]["cplocations"])
...
def process_signature(root, sig_path, first_method, second_method, output_directory, conf):
            ...
            for second_method_file in os.listdir(second_method_conf_dir):
                ...
                if os.path.isfile(second_method_file_path) and second_method_file_path.endswith('.json'):
                    ...
                    if first_method_json['status'] == 'SUCCESS' and second_method_json['status'] == 'SUCCESS':
                        result_json = merge_files(first_method_json, second_method_json, first_method, second_method)
...
def main():
    ...
    for signature_path in os.listdir(root):
        process_signature(root, signature_path, args.first_method, args.second_method, args.output_directory, 'best')
        process_signature(root, signature_path, args.first_method, args.second_method, args.output_directory, 'default')
