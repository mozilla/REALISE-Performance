...
# Fixed hyperparameter values used to pre-filter configurations before selecting the best
FIXED_HYPERPARAMS = {
    "max_back_window": 24,
    "min_back_window": 12,
    "fore_window": 12,
...
def matches_fixed_hyperparams(args: dict) -> bool:
    """Return True if the given args dict matches all FIXED_HYPERPARAMS values."""
    for key, fixed_value in FIXED_HYPERPARAMS.items():
        if key not in args:
            return False
        # Compare as the same type; args values may be int or float
        try:
            if type(fixed_value)(args[key]) != fixed_value:
                return False
...
def process_best(method):
    ...
    for dataset_metrics in datasets_metrics:
        for unit_method in dataset_metrics["results"][method]:
            ...
            # Only consider configurations that match the fixed hyperparameter values
            if not matches_fixed_hyperparams(conf):
                continue
    ...
    if not hyperparams:
        print(f"Warning: No configurations matched the fixed hyperparameters for method '{method}'. "
              f"Fixed values: {FIXED_HYPERPARAMS}")
...
def process_oracle(method):
    ...
    for dataset_metrics in datasets_metrics:
        ...
        for unit_method in dataset_metrics["results"][method]:
            # Only consider configurations that match the fixed hyperparameter values
            if not matches_fixed_hyperparams(unit_method["args"]):
                continue
            if unit_method["status"] == "SUCCESS":
                metrics = unit_method["scores"]
                f1 = metrics["f1"]
