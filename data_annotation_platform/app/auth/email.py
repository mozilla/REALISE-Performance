...
def send_password_reset_email(user):
    ...
    send_email(
        ...
        text_body=render_template(
            "email/reset_password.txt", user=user, token=token, dns_name=DNS_NAME
        ),
        html_body=render_template(
            "email/reset_password.html", user=user, token=token, dns_name=DNS_NAME
        ),
    )
...
def send_email_confirmation_email(user):
    ...
    send_email(
        ...
        recipients=[user.email],
        text_body=render_template(
            "email/confirm_email.txt", user=user, token=token, dns_name=DNS_NAME
        ),
        html_body=render_template(
            "email/confirm_email.html", user=user, token=token, dns_name=DNS_NAME
        ),
    )
